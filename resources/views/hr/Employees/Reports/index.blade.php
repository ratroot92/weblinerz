...
<button class="btn btn-secondary float-left" id="navigatable_btn_1"><i class=" text-white font-weight-bold fas fa-arrow-down mr-2"></i>Reports</button>

<a data-toggle="add_new_repo" href="{{ url('/add_employee_reports_create') }}" class="btn btn-primary add_report_button">Add New Report</a>
...
<div class="table-responsive top_padd" id="navigatable_div_1">
  <table class="table table-striped" id="table1" style="padding-top: 16px;">
      ...
      <tbody>
      @php $i=1; @endphp
          @foreach ($employeesReports as $reports)             
                  <tr>
                      <th scope="row">{{ $i++ }}</th>
                      <td>{{ $reports->name }}</td>
                      ...
                      <td> 
                          <a data-toggle="edit_reports" href="{{ url('/editview/'.$reports->id) }}"><img src="{{ asset('images/300-min.png') }}" alt=""></a>
                          <a href="{{ url('/employeereports/'.$reports->id) }}"><img src="{{ asset('images/400-min.png') }}" alt=""></a>
    ...
    <div class="modal-dialog modal-dialog-centered" id="add_modal" data="toggle" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Add Reports</h5>
...
<script type="text/javascript">
  ...
  $(document).ready(function () {
   ...
   //---------------------Edit form Modal Popup-----------------
    $(document).ready(function () {
        $(document).on('click','a[data-toggle=edit_reports]',function (event) {
    ...
    $(document).ready(function () {
        $(document).on('click','a[data-toggle=add_new_repo]',function (e) {
            e.preventDefault();
