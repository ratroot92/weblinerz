<form enctype="multipart/form-data" action="{{ url('/employeedocuments/update_employeedocument/') }}" name="editEmployeesDocuments" method="post" id="editEmployeesDocuments" class="form-container formdata editform">
    @csrf
    <div id="alrtmsg1"></div>
    <input type="hidden" name="saldraftID" value="{{$emp_documents->id}}">

    <div class="form-group">
        <label for="name">Name</label>
        <input type="text" id="name" class="name_documents" placeholder="Name" name="name" value="{{ $emp_documents->name }}" >
    </div>

    <div class="form-group">
        <label for="description">Description</label>
        <textarea name="description" class="name_documents" id="description" rows="3" style="margin: 0px; width: 392px; height: 110px;">{{ $emp_documents->description }}</textarea>
    ...
    <div class="form-group">            
        ...
        <select name=status id="status" class="name_documents">
            @if ($emp_documents->status=="active")
                <option value="active" selected>Active</option>
                <option value="inactive">Inactive</option>
            @elseif($emp_documents->status=="inactive")   
...
$('#editEmployeesDocuments').submit(function () {
